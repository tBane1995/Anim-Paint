...
void Animation::addEmptyFrame(sf::Vector2i size) {
	...
	layers.push_back(std::make_shared<Layer>(L"Layer " + std::to_wstring(layers.size()+1), sf::Vector2i(size.x, size.y), sf::Color::White));
	layers.push_back(std::make_shared<Layer>(L"Layer " + std::to_wstring(layers.size()+1), sf::Vector2i(size.x, size.y), sf::Color::Transparent));
