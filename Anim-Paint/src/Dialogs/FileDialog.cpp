...
LocationRect::LocationRect(std::wstring path, int depth = 0) : 
	...
	_text(basicFont, L"", basic_text_size) {
...
LocationAndFilesSeparator::LocationAndFilesSeparator(int linesCount) {
	...
	setSize(sf::Vector2f(file_dialog_separator_width, linesCount * basic_text_rect_height));
	...
	_filenameText(basicFont, L"File name", basic_text_size)
{
	...
	_bottomRect.setFillColor(dialog_content_rect_color_2);
...
void FileDialog::createLeftPanel(int dictionariesCount) {
	...
	_leftRect = sf::RectangleShape(sf::Vector2f(100.0f, (dictionariesCount) * basic_text_rect_height));
	...
	for (int i = 0; i < _locations.size(); i++) {
		_locations[i]->setSize(sf::Vector2f(_leftRect.getSize().x, basic_text_rect_height));
...
void FileDialog::createSeparator(int linesCount) {
	...
	_separator->_func = [this]() {
		...
		for (int i = 0; i < _files.size(); i++)
			_files[i]->setSize(sf::Vector2f(_rightRect.getSize().x, basic_text_rect_height));
...
void FileDialog::createRightPanel(int linesCount) {
	...
	_rightRect = sf::RectangleShape(sf::Vector2f(w, linesCount*basic_text_rect_height));
	_rightRect.setFillColor(sf::Color(255, 47, 47, 127));

	// files
	for (int i = 0; i < linesCount+1; i++) {
		FileRect* file = new FileRect();
		file->setSize(sf::Vector2f(_rightRect.getSize().x, basic_text_rect_height));
...
void FileDialog::setPosition(sf::Vector2f position) {
	...
	sf::Vector2f filenameInputPos(sf::Vector2f(getContentPosition().x + _filenameText.getGlobalBounds().size.x + 2 * dialog_padding, _rightRect.getPosition().y + _rightRect.getSize().y + dialog_padding));
...
void FileDialog::drawBottomPanel() {
	sf::View view(sf::FloatRect(
		{ _bottomRect.getPosition().x, _bottomRect.getPosition().y },
		{ _bottomRect.getSize().x, _bottomRect.getSize().y }
	));

	sf::FloatRect vp(
		{ _bottomRect.getPosition().x / mainView.getSize().x, _bottomRect.getPosition().y / mainView.getSize().y },
		{ _bottomRect.getSize().x / mainView.getSize().x, _bottomRect.getSize().y / mainView.getSize().y }
	);
	view.setViewport(vp);
